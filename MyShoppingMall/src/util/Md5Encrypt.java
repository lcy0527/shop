...
public class Md5Encrypt {
	...
	try {
		String pwd = getEncryptedPwd("1235");//获得加密后的16进制
		byte[] bs = hexStringToByte(pwd);//16进制的字符串转换成数组
		String q = byteToHexString(bs);//baey数组转换成16进制字符串
		System.out.println(q);
		String pwds = getEncryptedPwd("1235");//获得加密后的16进制
		byte[] bss = hexStringToByte(pwd);//16进制的字符串转换成数组
		String qs = byteToHexString(bs);//baey数组转换成16进制字符串
