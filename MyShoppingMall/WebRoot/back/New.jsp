...
<link rel="stylesheet" href="/MyShoppingMall/back/css/x-admin.css"
	media="all">
...
<body>
	...
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="new_add('添加用户','/MyShoppingMall/back/new_add.jsp','600','500')">
		...
		<table class="layui-table">
			...
			<tbody id="x-img">
				<c:forEach items="${newsList}" var="news">
					<tr>
						<td><input type="checkbox" value="${news.id}" name="">
						</td>
						<td>${news.id}</td>
						<td>${news.newstype}</td>
						<td>${news.title}</td>
						<td align="left" name=img><img
							src="/MyShoppingMall/back/images/img1.jpg" alt="" width="56"
							height="56">
						</td>
						<td>${news.createTime}</td>
						<td>${news.content}</td>
						<td class="td-status"><span
							class="layui-btn layui-btn-normal layui-btn-mini"> 已显示 </span>
						</td>
						<td class="td-manage"><a style="text-decoration:none"
							...
							onclick="new_edit('修改','news?method=findById&id=${news.id}','4','','510')"
							class="ml-5" style="text-decoration:none"> <i
								class="layui-icon">&#xe642;</i> </a> 
								<a title="删除"
							href="javascript:;" onclick="banner_del(this,'1', ${news.id})"
	...
	<br />
	<br />
	<script src="/MyShoppingMall/back/lib/layui/layui.js" charset="utf-8"></script>
	<script src="/MyShoppingMall/back/js/x-layui.js" charset="utf-8"></script>
	<script>
		...
		/*删除*/
		function banner_del(obj, id,newid) {
			layer.confirm('确认要删除吗？', function(index) {
				//发异步删除数据
			...
			window.location.href="news?method=del&id="+newid;
		}
	</script>
	<script>
		var _hmt = _hmt || [];
